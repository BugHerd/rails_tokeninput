...
var methods = {
  ...
  resetData: function(data) {
    if(this.data("settings") && this.data("settings").local_data) {
      this.data("settings").local_data = data;
    }
  }
};
...
$.TokenList = function (input, url_or_data, settings) {
  ...
  var input_box = $("<input type=\"text\"  autocomplete=\"off\">")
    ...
    .keydown(function (event) {
      ...
      switch(event.keyCode) {
        ...
        case KEY.ESCAPE:
          if(!$(this).val() && !selected_dropdown_item) {
            return true;
          }
          else {
            hide_dropdown();
  ...
  function delete_token (token,disableCallback) {
    ...
    if($.isFunction(callback) && !disableCallback) {
  ...
  function populate_dropdown (query, results) {
    if(results && results.length) {
      ...
      dropdown.height(27 * results.length);
    ...
    if(cached_results) {
      ...
      if($(input).data("settings").url) {
        ...
        if(query) {
          ...
          results = _.toArray($(input).data("settings").local_data);
        }
